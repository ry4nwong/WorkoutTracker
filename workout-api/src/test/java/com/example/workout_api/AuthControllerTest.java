...
public class AuthControllerTest {
    ...
    void testRegisterUser() throws Exception {
        when(authService.createUser(user)).thenReturn(user);
    ...
    void testRegisterUserInvalidUsername() throws Exception {
        when(authService.createUser(user)).thenThrow(UsernameAlreadyExistsException.class);
        mockMvc.perform(post("/api/auth/register")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(user)))
    ...
    void testRegisterUserInvalidEmail() throws Exception {
        when(authService.createUser(user)).thenThrow(EmailAlreadyExistsException.class);
        mockMvc.perform(post("/api/auth/register")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(user)))
            .andExpect(status().isConflict());
